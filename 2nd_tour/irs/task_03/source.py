...
def lineToSegments(begin, end, segments = 2):
    # begin, end - массивы с координатами отрезка (начало, конец). начало, 
    # как правило - центр
...
def rgb2grey(pic_in, mode):
    ...
    for i in range(pic_h):
        ...
        for j in range(pic_w):
            ...
            if mode == 1:               
                # "V from HSV", V = max(r, g, b)
                result = max(RGB)
            elif mode == 2:            
                # через Y ' from Y'UV(средневзвешенное) вариант 1
                # result = Math.round((0.299 * RGB[0]) + (0.587 * RGB[1]) + (0.114 * RGB[2]))
                result = round((0.2126 * RGB[0]) + (0.7152 * RGB[1]) 
                    + (0.0722 * RGB[2]))
            elif mode == 3:  
                # через Y ' from Y' UV(средневзвешенное) вариант 2
                clr = pic_in[i][j]
                tmp = ((((clr >> 16) & 0xff) * 76) + (((clr >> 8) & 0xff) * 150) 
                    + ((clr & 0xff) * 29)) >> 8
                result = tmp << 16 | tmp << 8 | tmp  # r | g | b
            elif mode == 4:         
                # Поиск ближайшей точки нейтральной оси: L = (max(R, G, B) + min(R, G, B)) / 2
                result = round((max(RGB) + min(RGB)) / 2)
            elif mode == 5:         
                # Среднее арифметическое компонент R, G, B
